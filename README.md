...
├── domain/                            Tipos espelhando as entidades do backend (User, Shortage, enums)
...
│   └── shortages/                     RegisterShortagePage, ShortagesQueuePage, ShortageCard, CancelShortageModal
├── components/                        Button, Input, Select, Modal, StatusBadge, Layout (nav por papel)
...
| `/faltas` | todos | Fila de faltas agrupada por status (Registrada, Em cotação, Comprada), com busca por código/nome. Vendedor só vê as próprias faltas (filtrado pelo backend); admin/comprador veem a fila completa. Botões de avançar status e cancelar aparecem conforme o papel e as regras de `docs/02-modelo-dominio.md` do backend. |
